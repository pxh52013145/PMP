...
impl Vst3Plugin {
    ...
    pub fn open_editor_window(&mut self, title: Option<&str>) -> Result<()> {
        unsafe {
            let title_cstring = title
                .map(|value| CString::new(value).map_err(|_| Error::Other("Invalid editor title".to_string())))
                .transpose()?;
            let title_ptr = title_cstring
                .as_ref()
                .map(|value| value.as_ptr())
                .unwrap_or(std::ptr::null());

            let result = ffi::rack_vst3_plugin_open_editor_window(self.inner.as_ptr(), title_ptr);
    ...
    pub fn close_editor_window(&mut self) -> Result<()> {
        unsafe {
            let result = ffi::rack_vst3_plugin_close_editor_window(self.inner.as_ptr());
            if result != ffi::RACK_VST3_OK {
                return Err(map_error(result));
            }
            Ok(())
    ...
    pub fn editor_is_open(&self) -> Result<bool> {
        unsafe {
            let result = ffi::rack_vst3_plugin_editor_is_open(self.inner.as_ptr());
            if result < 0 {
                return Err(map_error(result));
            }
            Ok(result != 0)

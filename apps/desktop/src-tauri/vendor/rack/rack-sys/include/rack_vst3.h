...
// ============================================================================
//
// NOTE: This is currently implemented on Windows only (HWND). Other platforms
// will return RACK_VST3_ERROR_NOT_SUPPORTED.

// Open the plugin's native editor in a standalone native window (best-effort).
// title: optional window title (can be NULL).
// Returns 0 on success, negative error code on failure.
int rack_vst3_plugin_open_editor_window(RackVST3Plugin* plugin, const char* title);

// Close the editor window if open. Safe to call multiple times.
// Returns 0 on success, negative error code on failure.
int rack_vst3_plugin_close_editor_window(RackVST3Plugin* plugin);

// Returns 1 if editor is open, 0 if not open, negative error code on failure.

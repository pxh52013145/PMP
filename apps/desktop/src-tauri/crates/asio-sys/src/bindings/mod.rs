...
impl Asio {
    ...
    pub fn load_driver(&self, driver_name: &str) -> Result<Driver, LoadDriverError> {
        ...
        // `ASIOInit` reads `asioVersion` + `sysRef` as input (see ASIO SDK `asio.h`).
        // Use a deterministic, safe initial value instead of uninitialized memory.
        let mut driver_info: ai::ASIODriverInfo = unsafe { std::mem::zeroed() };
        driver_info.asioVersion = 2;
        driver_info.sysRef = default_sys_ref();
        ...
        unsafe {
            ...
            match ai::load_asio_driver(driver_name_cstring.as_ptr() as *mut i8) {
                ...
                true => {
                    // Initialize ASIO.
                    asio_result!(ai::ASIOInit(&mut driver_info as *mut _))?;

...
import { RUNTIME_EVENT_NAMES } from './runtimeEventChannel';
...
export function buildWorkerBootstrapSource(runtimeHello: RuntimeHello): string {
  return `(${runtimeBridgeWorkerBootstrap.toString()})(${JSON.stringify(runtimeHello)}, ${JSON.stringify(RUNTIME_EVENT_NAMES)})`;
}

function runtimeBridgeWorkerBootstrap(

...
export default function WindowResizeHandles() {
  ...
  const startResize = async (event: React.PointerEvent<HTMLDivElement>, direction: ResizeDirection) => {
    ...
    const flushMove = () => {
      ...
      pendingEvent = null;
      void appWindow.setPosition(new LogicalPosition(next.x / session.scaleFactor, next.y / session.scaleFactor));
      void appWindow.setSize(new LogicalSize(next.width / session.scaleFactor, next.height / session.scaleFactor));
    };
    ...
    <div className="window-resize-handles" aria-hidden="true">
      {DIRECTIONS.map((direction) => (
        <div
          ...
          onPointerDown={(event) => void startResize(event, direction)}
        />
      ))}
    </div>
  );
